...
In this section, a country's "happiness" is quantified by aggregating the values of 6 key criteria: GDP per capita, social support, healthy life expectancy, freedom, generosity, and absence of corruption. This section of the report critically analyses these six variables to determine if happiness does change, according to the quality of the society people live across the world.
...
# setdiff(happiness_2019$Country, ctry_region$Country) 
...
# Clean names
happiness_2019 <- full_join(happiness_2019, ctry_region, by = "Country") %>%
  clean_names()
```
...
setdiff(happiness_2019$country, map_world$region)
```

```{r cleaning-ctry-names}
# Create new df for map
happiness_map <- happiness_2019
...
happiness_attr2019 <- happiness_2019 %>%
  filter(year == 2019) %>%
...
p2 <- ggplot(data = happiness_attr2019,
...
# Interactive map
# ggplotly(p2, tooltip = c("colour","label", "y", "size"))
```
