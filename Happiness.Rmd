...
\end{align*}
  ...
  Instead of using the factors in the given dataset, we join other datasets to analysis the happiness score, including GDP, CPI, population and area by different countries.There are another three datasets such as GDP, CPI and population data which are downloaded from The World Bank. And area of each country is computed using the simple feature polygons with the package `rnaturalearth` (@rnaturalearth). We select all the happiness scores between 2015-2019 to construct a new table for happiness score with new factors, therefore **Table \@ref(tab:table-FT)** and **Table \@ref(tab:table-LT)** can be displayed the first three data and the last three data about happiness scores in different years. In addition, we pick up thee variables such as gdp,cpi and population between 2015-2019 to put them into new happiness score dataset. After matching the area of each country and joining them into one dataset,the new dataset with happiness score for analyzing the relative variable improtance is established in the **Table \@ref(tab:tableHS)**.
```{r lmmodel}
...
## Variable importance
  ...
  Firstly, we built up a **LM** model for the new happiness score dataset. By avoding the different scaling for each variables and checking the percentage marginal effect of each variable, using logarithmic method for setting up a linear regression model. Therefore, a **LM** model is shown :
\begin{align*}
\text{log(Happiness Score)} = & \text{ log(CPI)} +  \text{ log(GDP)}+\text{ log(Area)}+ \\
& \text{ log(Population)} 
\end{align*}
  From**Table \@ref(tab:variable-important)**,we can see that the total proportion of variance explained by the model with all four predictors ($$R^2$$) is around 72.88%. For the four predictors, GDP with the higest value in four predictors contributed to 64.37% for the happiness scores,indicating that GDP factor is the most important for hanppiness score and it is related to the change of happiness scores(@gromping2015variable). On the other hand,by using **variance inflation factor** (VIF) measure to evaluate the impact of the correlation among the explanatory variables. In the **Table \@ref(tab:mulitilinearity)**, the VIF value in each predictor is not too high but for the population is almost at 3, indicating that there is a slight correlation of population with other variables in the model(@daoud2017multicollinearity).
