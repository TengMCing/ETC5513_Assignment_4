...
comparison_2015 <- happiness_2015 %>% 
  ...
  mutate(above_average = length(Region), below_average = total_countries - above_average)
...
comparison_2015 %>% 
  ...
  theme_void() + 
  geom_text(aes(x = 1, y = cumsum(percentage) - percentage/2, label = percentage_label)) + 
