...
@helliwell2019world has indicated that there is an inner connection between government and happiness, it means the jobs of government would influence the happiness of citizens, while the people feeling of happiness could guide them to decide which kind of government to support. In addition, @helliwell2019world has also explained that the living quality of people could be measured by the happiness of each country. 

This report will utilized the happiness score of each country to analyse the changing of well-being according to countries, regions, and the world. The happiness scores will be allocated into a world map to explain the stark differences in happiness scores across the world. Subsequently, six key variables relating to socioeconomical factors are critically analysed to explain the sources of variations in happiness among countries.  

The report will also analyse the importance of each modelling variables, as well as the relationships between happiness scores and other factors. In the study, this report will illustrate the trends over five years based on the top ten countries in 2015, and compared the distribution of the number of above average-happiness-level countries of each region in 2015 and 2019. Meanwhile, this report will explore the relative importance variables for modelling happiness scores and the correlation between happiness scores and five factors. Furthermore, the relationship between predictors and happiness score in linear regression analysis, that will be analysed and showed in the last section.
...
We downloaded the data from Kaggle with the license of CCO:Public Domain, which contain the different factors and happiness scores between 2015-2019. In the five datasets, the variable for evaluating the happiness scores about GDP, health, freedom, generosity and government corruption are consistent but the family factor has been changed into social support after 2017. Also, the datasets are ranked from the highest one to the lowest one and the average of the observations are around 156 countries. The happiness scores are based on answered the main life evaluation in the Gallup World Poll. In this report, we are going to use these five datasets to analyse a series of questions for the happiness scores. In addition, other datasets from the World Bank will be used because of some issues with the happiness scores report. Details about those datasets will be discussed in the modelling section.

## Data pre-processing

The happiness data downloaded from Kaggle is clean. No major data cleaning process needs to be made. Additional pre-processing steps will be discussed in each section if needed.
...
In this section, six key criteria, which encompass socioeconomic factors of countries are quantified to explain the variation of happiness across countries. These criteria include GDP per capital, social support, healthy life expectancy, freedom, generosity, and the absence of corruption. This section critically analyses these six variables to determine if happiness does change, according to the quality of the society people live across the world.

### Incorporate the map data
...
To construct the happiness score on a map, the world map data is taken from the maps `R` package [@maps] to create a base map and retrieve the geographic coordinates of all countries in the world. Next, the happiness data is further wrangled to resolve conflicting country names between the happiness and the map data; Both datasets are then joined to build the world map with the happiness score of each country.

The attribute scores of the six criteria are evaluated across the various countries and regions. Further, the estimates of the world population data are employed [@wbstats] to determine if population size affects happiness scores. Similar to the world map, conflicting country names are modified so countries in both datasets match. Consequently, the six attribute scores across all countries in the happiness data set are laid out on a scatter plot.
...
### 2019 Happiness Scores on the World Map
...
### Comparison between attribute scores across countries
...
Figure \@ref(fig:happiness-attr2019) shows a large disparity between the top and bottom countries in each criterion. In conjunction with figure \@ref(fig:world-happiness-map), countries with high happiness scores like countries the Scandinavian region has the highest values for most of the key variables. Therefore, it is suggested that countries who live happier lives are likely to live longer, have more social support, are more cooperative, and generally more able to meet life’s demands with a higher GDP.
...
Healthy life expectancy are noticeably similar in each region. Conversely, large variations are illustrated within regions in most of the other attributes. For instance, in 'GDP per capital' and 'freedom', only approximately half of Central and Eastern Europe were above average, signifying that attribute scores are not homogeneous across all countries in a given region. 

Importantly, the aforementioned variables may be taking credit due to unmeasured factors. The next section of this report will investigate these key criteria to see if happiness scores can be accurately attributed or explained by these variables of the different countries.

\clearpage

# Modelling

## Which variables are useful for modelling happiness scores and how the correlation between happiness scores and other factors?
...
\end{align*}
  ...
  Instead of using the factors in the given dataset, we join other datasets to analysis the happiness score, including GDP, CPI, population and area by different countries. There are another three datasets such as GDP, CPI and population data which are downloaded from The World Bank [@world2020world]. And area of each country is computed using the simple feature polygons with the package `rnaturalearth` [@rnaturalearth]. In addition, we pick up thee variables such as GDP,CPI and population between 2015-2019 to put them into new happiness score dataset. After matching the area of each country and drop out the missing value, the new dataset with happiness score for analyzing the relative variable importance is established in the **Table \@ref(tab:tableHS)**.
...
### Variable importance
  ...
  Firstly, we built up a linear regression model for the new happiness score dataset. By eliminating the heteroskedasticity in the residual and interpreting the percentage marginal effect of each variable, using logarithmic method for setting up a linear regression model is an appropriate way. Therefore, a linear regression model is shown :
\begin{align*}
\text{log(Happiness Score)} = & \text{ CPI} +  \text{ log(GDP)}+\text{ log(Area)}+ \text{ log(Population)} 
\end{align*}

  From **Figure \@ref(fig:variable-important)**,we can see that the total proportion of variance explained by the model with all four predictors is around 72.64%. For the four predictors, GDP with the highest value in four predictors contributed to above 65% for the happiness scores,indicating that GDP factor is the most important for happiness score and it is more likely to affect the change of happiness scores when the GDP variable change [@gromping2015variable]. On the other hand,by using variance inflation factor (VIF) measure to evaluate the impact of the correlation among the explanatory variables. In the **Table \@ref(tab:mulitilinearity)**, the VIF value in each predictor is not too high but for the population variable is the highest one around at 3, indicating that there is a slight correlation of population with other variables in the model [@daoud2017multicollinearity].

### Scatter matrix
  To analyze the relationship between different predictors, we use scatter matrix which can be visualized easily how much one variable is affected by another or the relationship between them. The **Figure \@ref(fig:scattermatrix)** compares all the predictors in the model and represents the relationship between these variables. If there is a strong linear positive correlation between the two factors, we can conclude that if one factor is important in assessing happiness scores, then another factor may also contribute to this. Based on the figure, it seems that the importance of GDP and area as well as population and area are closely related. Therefore,GDP is the most important factors for evaluating the happiness scores as it has the highest positive correlation with happiness score,but also for the developed countries in most of western area with highest GDP, the happiness scores will be higher than the development countries with lower GDP.
...
```{r variable-important,fig.cap="Variable importance in happiness score",fig.width = 10, fig.height = 10, fig.alig = "center"}
...
```{r scattermatrix,fig.cap="Happiness Score Scatter Matrix", fig.width = 10, fig.height = 10,fig.alig = "center"}
...
## What is the relationship between predictors and happiness score in linear regression analysis?
...
We use variables mentioned in the previous section as predictors, the regression result is shown in **Table** \ref{tab:lmcoeff}. According to the result, we can write down the formula for this model:
...
According to the overall studies, the report has concluded that Happiness scores within each continents are analogous, where the top 10 happiest countries are evidently dominated by Scandinavian countries. On the contrary, Sub-Saharan happiness scores are relatively low compared to the rest of world, which may be attributed to lower socioeconomic levels [@sachs2018world]. However, Europe is still the continent has the largest number of above-average countries over five years, and Western Europe was also the dominant region in this period. In fact, Europe has owned the highest number of developed countries compared with other continents, which might imply a possibility that the people in developed countries might more satisfy their countries. Therefore, the environment and condition of developed countries might guide leaders of other countries to understand the factors which would improve the sense of happiness of their citizens. which would improve the sense of happiness of their citizens.

On the other hand, the concept of happiness of each people would keep changing over years, the main reason might because the environment of each country has influence the sense of happiness of citizens. The countries with higher happiness scores are associated with healthier longevity, having better social support and better perceptions of government, and are usually more able to meet life’s demands. Albeit this, many countries with lower happiness scores had high scores in 'generosity' and 'freedom', suggesting that other key variables may be diminishing the happiness scores of these countries. However, it should be noted that the aforementioned variables may be taking credit due to unmeasured factors.

In addition, based on analyzing the relative variable importance in modeling the happiness scores with four factors which are GDP, CPI,area and population, it has been indicated that GDP is the most significant factors. Also, the area variable is strongly related with the GDP which contributes to happiness score as well. Therefore, GDP is an essential factor for evaluating the happiness score, especially for rich countries which GDP dominates the happiness scores comparing with other factors.

Furthermore, GDP has a positive marginal effect on happiness score, while the population has a negative marginal effect on happiness score. Although heteroskedasticity, endogeneity and observability issues arise, the happiness score can still be well explained by our simple linear model.

\clearpage

# Acknowledgement

The programming language used to analyse the happiness data is R (3.5.3) [@R]. The latex version to generate this report is "pdfTeX 3.14159265-2.6-1.40.21 (TeX Live 2020/W32TeX) kpathsea version 6.3.2".

Following packages has been included in our scripts:

- package ggplot2 (3.2.0) [@ggplot2],
- package tidyverse (1.2.1) [@tidyverse],
- package dplyr (1.0.0) [@dplyr],
- package relaimpo (2.2.3) [@relaimpo],
- package GGally (1.4.0) [@GGally],
- package kableExtra (1.1.0) [@kableExtra],
- package rnaturalearth (0.1.0) [@rnaturalearth],
- package sf (0.9.2) [@sf],
- package rgeos (0.4.2) [@rgeos],
- package lwgeom (0.2.1) [@lwgeom],
- package car (3.0.2) [@car],
- package here (0.1) [@here],
- package lubridate (1.7.4) [@lubridate],
- package cowplot (1.0.0) [@cowplot],
- package janitor (1.2.1) [@janitor],
- package maps (3.3.0) [@maps],
- package wbstats (0.2) [@wbstats],
- package plotly (4.8.0) [@plotly],
- package knitr (1.28) [@knitr],
- package rmarkdown (2.1) [@rmarkdown],
- package tinytex (0.22) [@tinytex]
